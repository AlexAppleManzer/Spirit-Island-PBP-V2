...
// Helper to load game state from database. Returns true if state was loaded.
const loadGameStateFromDb = async (gameId: string, ydoc: Y.Doc): Promise<boolean> => {
  try {
    const gameIdNum = parseInt(gameId, 10);
    if (isNaN(gameIdNum)) {
    ...
    console.warn(`[DB] No snapshot found for game ${gameIdNum} — game may not have been initialized`);
    return false;
  } catch (err) {
...
const getGameDoc = async (gameId: string): Promise<Y.Doc> => {
  ...
  const promise = (async () => {
    ...
    // Load persisted state from DB. State must already exist — it is generated by
    // POST /api/games at creation time. If missing (e.g. server crashed before
    // the initial save completed) the doc stays empty; the client's synced-event
    // handler will surface an error rather than silently showing a blank board.
    const stateLoaded = await loadGameStateFromDb(gameId, ydoc);
    if (stateLoaded) {
      // Fill in any fields that were added after this game was created.
      const initialPlayerCount = await getInitialPlayerCountFromDb(gameId);
      ensureGameDefaults(gameMap, initialPlayerCount);
    }

    attachSaveListener(gameId, ydoc);
...
app.post('/api/games', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    const gameId = String(createdGame.id);

    // Generate the complete base gamestate on the server now, before any client
    // connects. Using selectedSpiritCount (from the form) ensures the correct
    // number of boards is created — not the DB player_ids count which is still 1.
    const initDoc = new Y.Doc();
    ensureGameDefaults(initDoc.getMap('game'), selectedSpiritCount, selectedAdversaryId, selectedAdversaryLevel);

    // Persist immediately (synchronous, no debounce) so that any subsequent
    // getGameDoc call (WebSocket connect, join, etc.) loads the correct state.
    await saveGameStateToDb(gameId, initDoc);

    // Register the doc in-memory and attach the debounced save listener for
    // future live updates. Do this directly instead of going through getGameDoc
    // so we reuse the doc we just initialized rather than reloading from DB.
    attachSaveListener(gameId, initDoc);
    gameDocs.set(gameId, initDoc);
